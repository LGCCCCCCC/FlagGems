...
def pixel_shuffle(input: torch.Tensor, upscale_factor: int) -> torch.Tensor:
    ...
    if upscale_factor <= 0:
        raise ValueError(
            f"upscale_factor must be positive, but got {upscale_factor}"
        )
    ...
    input_reshaped = input.reshape(
        *batch_dims, channels_out, r, r, height_in, width_in
    )
    ...
    output = input_permuted.reshape(
        *batch_dims, channels_out, height_out, width_out
    )
